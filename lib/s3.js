 ...
 *
 */
S3.prototype.uploadFileToBucket = function(host, fileNameWithPath, stream){
	try {
		...
		fs.stat(fileNameWithPath, function(err, stats){
			...
			s3._getBucketName(host, function(res){
				...
				var afterMD5Callback = function(hash){
					...
					s3._sendPutRequest(headers, fileNameWithPath, stream);
				};
 ...
 *
 */
S3.prototype._sendPutRequest = function(headers, fileNameWithPath, stream){
	...
	dns.resolve4(headers.Host,function (err, addresses) {
		...
		var fileStream = stream || s3._createReadFileStream(fileNameWithPath);
