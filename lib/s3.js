 ...
 *  Bill Casarin
 *
 *  A simple S3 library written because the other implementation doesn't meet 
 *  needs and is outdated.
 *
 *  What this library is good for:
 *
 *  ** Can send files any size (although S3 is currently limited at 5 gigs)
 *  ** Streams files from disk to socket -- minimal memory footprint and very 
 *     efficient
 ...
 *  ** Supports amazons new REDUCED_REDUNDANCY storage (cheaper but less 
 *     reliable)
...
var S3 = function(awsSecretKey, awsAccessKey, options){
  ...
  this._checkMD5 = options.checkMD5 === undefined ? true : options.checkMD5;
...
S3.prototype.uploadStream = function (host, stream, destName, contentLength,
                                      mimeType, md5) {
  return this._upload(host, "", destName, contentLength, stream, mimeType, md5);
}

S3.prototype._upload = function(host, fileName, destName, contentLength, stream,
                                mimeType, md5) {
  try {
    var awsSecretKey = this._awsSecretKey;
    var awsAccessKey = this._awsAccessKey;

    // use fileName if destName is not supplied
    ...
    if (stream) {
      if (!mimeType)
        s3.emit("error", "mimeType not specified for stream upload");

      if (s3._checkMD5 && !md5) {
        s3.emit("error", 
          "checkMD5 is true but no md5 was specified for the stream");
        isInvalid = true;
      }

      if (contentLength === undefined) {
        s3.emit("error", "contentLength must be specified with streams");
        isInvalid = true;
      }
    }
    ...
    function uploadWithSize(size) {
      mimeType = mimeType || s3._getMimeType(destName);

      s3._getBucketName(host, function(res){
        var bucketName = res;

        var resource = '/' + bucketName + '/' + destName;
        var headers = s3._getPutHeaders(host, mimeType, size);
        ...
        var afterMD5Callback = function(hash){
          ...
          s3._addAuthorizationHeader(headers, 'PUT', resource, awsSecretKey,
                                     awsAccessKey);
          // send the put request
          s3._sendPutRequest(headers, fileName, destName);
        };

        // add md5 if set in options
        if(s3._checkMD5 === true) {
          if (md5) {
            afterMD5Callback(md5);
          } else {
            s3._getMD5OfMessage(s3, fileName, size, afterMD5Callback);
          }
        } else{
          afterMD5Callback();
        }
    ...
    if (stream) {
      ...
      fs.stat(fileName, function(err, stats) {
        if (err) {
          s3.emit(err);
          return;
        } // else we got the size
        uploadWithSize(s3._getFileSize(stats));
      });
    }
 ...
 *     "prefix/file.txt" maps to "s3://bucket/prefix/file.txt"
 *
 * mimeType [optional]:
 *   The mime type is determined from the destination file extension using the 
 *   mime node library, otherwise it uses this parameter
 *
 * md5 [optional:
 *   A precomputed md5. This is optional, the library will generate an
 *   md5 for you if this is not supplied
 */
S3.prototype.uploadFile = function (host, fileName, destName, mimeType, md5) {
  return this._upload(host, fileName, destName, undefined, undefined, mimeType,
                      md5);
}

// Compatibility
S3.prototype.uploadFileToBucket = S3.prototype.upload;
...
S3.prototype.getFileFromBucket = function(host, fileName, destinationFile){
  try {
    ...
    s3._getBucketName(host, function(res){
        ...
        // add the amazon authorization header
        s3._addAuthorizationHeader(headers, 'GET', resource, awsSecretKey, 
                                   awsAccessKey);
 ...
 *
 */
S3.prototype._sendPutRequest = function(headers, fileNameWithPath, destName, 
                                        fStream) {
  var s3 = this;
  ...
  dns.resolve4(headers.Host,function (err, addresses) {
    ...
    var fileStream = fStream || s3._createReadFileStream(fileNameWithPath);

    // don't pull anything from the file until we have a connection with the
    // remote server
    fileStream.pause();

    var progressBar = 
      s3._progressBar(s3, 'progress', headers['Content-Length']);
    ...
    stream.on('connect', function(streamConnection){
      var headerString = "PUT " + '/' + destName + " HTTP/1.1" + "\n"
