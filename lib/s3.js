 ...
 *
 *  PUT :
 *    progress(percentsent){}, complete(hadError){}, error(exception){}
 *  DELETE :
...
S3.prototype.uploadStream = function (host, destName, stream, contentLength
                                      mimeType) {
  return this._upload(host, "", destName, contentLength, stream, mimeType);
}

S3.prototype.uploadFile = function (host, fileName, destName, mimeType) {
  return this._upload(host, fileName, destName, mimeType);
}
...
S3.prototype._upload = function(host, fileName, destName, contentLength, stream
  ...
  try {
    ...
    function uploadWithSize(size) {
      mimeType = mimeType || s3._getMimeType(fileNameWithPath);
    ...
    if (stream) {
     ...
     // check if file is valid... otherwise bail out.
      fs.stat(fileNameWithPath, function(err, stats){
        ...
        uploadWithSize(this._getFileSize(stats));
      });
    }
...
S3.prototype._sendPutRequest = function(headers, fileNameWithPath) {
  ...
  dns.resolve4(headers.Host,function (err, addresses) {
    ...
    var progressBar = s3._progressBar(s3, 'progress',
    ...
    stream.on('close', function(hadError){ s3.emit('complete', hadError); });
